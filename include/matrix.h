...
public:
        ...
        matrix<T, 1, cols> row(std::size_t r) {
                return {rdata + r * cols};
        }

        matrix<T, rows, 1> col(std::size_t c) {
                return {cdata + c * rows};
        }
        ...
        T dot(const matrix<T, rows, 1> &vector) {
                if (cols != 1) {
                        throw std::invalid_argument(
                                "Dot product is only defined for vectors (must have exactly 1 column)"
                        );
                }
                T result = 0;
                for (std::size_t i = 0; i < elements; ++i) {
...
private:
        ...
        friend std::ostream &operator<<(std::ostream &lhs, const matrix &rhs) {
                for (std::size_t r = 0; r < rows; ++r) {
                        lhs << "[";
                        if (cols > 0) lhs << rhs.rdata[r * cols];
